...
const TRANSLATIONS = {
  en: {
    ...
    index_proj_desc: "a single endpoint that combines multiple roblox apis to fetch user profiles, groups, avatar, presence, and more. includes a geometry dash easter egg 🎮",
  ...
  ru: {
    ...
    index_proj_title: "Roblox User Info API",
    index_proj_desc: "единый эндпоинт, объединяющий несколько Roblox API для получения профилей, групп, аватаров, статуса и многого другого. включает пасхальное яйцо для Geometry Dash 🎮",
  ...
  ja: {
    ...
    index_proj_title: "Roblox User Info API",
    index_proj_desc: "複数のRoblox APIを1つのエンドポイントにまとめて、プロフィール、グループ、アバター、在線状況などを取得できます。Geometry Dashのイースターエッグ付き 🎮",
